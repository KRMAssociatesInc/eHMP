...
RpcSender.prototype.connect = function(callback) {
    ...
    self.logger.debug('Sender.connect()');

    self.close();

    self.socket = this.createSocket();
    self.setTimeout(self.config.connectTimeout);
...
RpcSender.prototype.send = function(rpcString, callback) {
    ...
    if (!self.socket) {
        return callback('no socket instance');
    }

    self.logger.debug('Sender.send(\'%s\')', makeVisible(rpcString));
    ...
    self.socket.on('data', function receive(data) {
        self.logger.trace('Sender.receive()');
    ...
    self.setTimeout(self.config.sendTimeout);
...
RpcSender.prototype.setTimeout = function(timeoutMillis) {
    if (this.socket) {
