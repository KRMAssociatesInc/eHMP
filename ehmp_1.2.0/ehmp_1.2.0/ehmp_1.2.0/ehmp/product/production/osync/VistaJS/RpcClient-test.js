...
RpcClient.prototype.execute = function execute(rpcCall, callback) {
    ...
    if (arguments.length < 2) {
        ...
        return setTimeout(callback('Insufficient number of arguments'));
    }

    if (_.isUndefined(this.sender) || _.isNull(this.sender)) {
        // TODO: attempt to connect automatically if this condition is met?
        return setTimeout(callback('RpcClient is not connected to the Vista server'));
    }
    ...
    if (!call) {
        return setTimeout(callback('Invalid arguments for rpcCall'));
    }
