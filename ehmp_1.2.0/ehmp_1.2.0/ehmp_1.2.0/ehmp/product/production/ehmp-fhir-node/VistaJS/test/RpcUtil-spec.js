...
describe('testCipherPad', function(test) {
	it('tests the CIPHER PAD length', function() {
		expect(RpcUtils.CIPHER_PAD.length).toEqual(20);
	});

	it('tests CIPHER PAD row length', function() {
		expect(_.every(RpcUtils.CIPHER_PAD, function(row) {
			return row.length == 94;
		})).toBe(true);
	});

	it('tests each character of CIPHER PAD', function() {
		expect(_.every(RpcUtils.CIPHER_PAD, function(row, rowIdx) {
...
describe('testRpcParameter', function(test) {
	...
	it('tests that a Literal RpcParameter is built correctly', function() {
		param = RpcUtils.buildLiteralParamString('SomeParam');
		expect(param).toEqual('0009SomeParamf');
	});

	it('tests that Encrypted RpcParameters are built correctly', function() {
		param = RpcUtils.buildEncryptedParamString('lgs413', 0, 1);
		expect(param).toEqual('0008 CTgF#b!f');

		param = RpcUtils.buildEncryptedParamString('lgs413', 1, 19);
		expect(param).toEqual('0008!S;|\\M&3f');

		param = RpcUtils.buildEncryptedParamString('lgs413', 12, 17);
...
describe('testParamList2String', function(test) {
	...
	var actual = RpcUtils.buildListParamString(param_list);
...
describe('strPack', function() {
	...
	it('pads string with zeros', function() {
		expect(RpcUtils.strPack('something!5here', 10)).toEqual(expected);
	});
});
...
describe('testPrependCount', function(test) {
	...
	it('prepends length to string', function() {
		expect(RpcUtils.prependCount('SomeString', 10)).toEqual(expected);
	});
});
...
describe('testAdjustForNumericSearch', function(test) {
	...
	it('tests the adjustment for numeric search', function() {
		expect(RpcUtils.adjustForSearch('245')).toEqual(expected);
	});
});

describe('testAdjustForStringSearch', function(test) {
	var expected = 'Snurc~';
	var actual = RpcUtils.adjustForSearch('Snurd');
...
describe('testAdjustForSearch', function(test) {
	var expected = 244;
	var actual = RpcUtils.adjustForSearch('245');
	it('tests adjustment for search with a number', function() {
		expect(actual).toEqual(expected);
	});

	expected = 'Snurc~';
	actual = RpcUtils.adjustForSearch('Snurd');
...
describe('testIsNumeric', function(test) {
	it('validates various numeric tests', function() {
		expect(RpcUtils.isNumeric('1234')).toBe(true);
		expect(RpcUtils.isNumeric('b1234')).not.toBe(true);
		expect(RpcUtils.isNumeric('1234b')).not.toBe(true);
		expect(RpcUtils.isNumeric('12b34')).not.toBe(true);
		expect(RpcUtils.isNumeric(' 1234')).not.toBe(true);
		expect(RpcUtils.isNumeric('1234 ')).not.toBe(true);
	});
});