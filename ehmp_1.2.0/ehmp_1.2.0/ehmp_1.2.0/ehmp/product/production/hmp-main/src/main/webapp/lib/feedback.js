...
// function to remove elements, input as arrays
        ...
        removeElement = function(el) {
            if (el.parentNode !== null ) { // check that the item was actually added to DOM
                el.parentNode.removeChild( el );
            }
        },

        removeHtmlCollection = function(rcoll) {
            while(rcoll.length>0) {
                removeElement(rcoll[0]);
            }
        },
...
//                    data = {
//                        screenshot: window.Feedback.Screenshot._data,
//                        title: window.Feedback.Form._data.title,
//                        description: window.Feedback.Form._data.description
//                    }
                    ...
                    adapter.send( data, function( resp ) {
                        ...
                        if ( resp.status==200 && resp.responseText ) {
                            ...
                            modalBody.appendChild( document.createTextNode( "Successfully created new ticket #"+json.key ) );
                        } else {
...
//        }
    ...
    window.Feedback.Screenshot.prototype.close = function(){
        ...
        removeHtmlCollection( document.getElementsByClassName( this.options.blackoutClass ) );
        removeHtmlCollection( document.getElementsByClassName( this.options.highlightClass ) );
    ...
    window.Feedback.XHR.prototype.send = function( data, callback ) {
        ...
        var appInfo = Ext.encode(gov.va.hmp.AppContext.getAppInfo());
        ...
        Ext.Ajax.request({
            ...
            failure: function(resp) {
                console.log("Oh no! "+resp.responseText);
                callback(false);
            }
        })
//        var xhr = this.xhr;
...
//
//        var stringed = window.JSON.stringify({data: data});
//        var sdat = encodeURIComponent(stringed);
//        xhr.send( "data=" + sdat);
