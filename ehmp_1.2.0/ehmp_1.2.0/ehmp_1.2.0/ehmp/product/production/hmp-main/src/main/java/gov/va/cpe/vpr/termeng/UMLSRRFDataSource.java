...
import gov.va.cpe.vpr.termeng.UMLSBuildPolicy.RowMatcher;
...
public class UMLSRRFDataSource extends AbstractTermDataSource {
	...
	protected void init(String tmpDir) {
		...
		MVStore s1 = new MVStore.Builder().fileName(f1).autoCommitDisabled().compressData().open();
		MVStore s2 = new MVStore.Builder().fileName(f2).autoCommitDisabled().compressData().open();
		MVStore s3 = new MVStore.Builder().fileName(f3).autoCommitDisabled().compressData().open();
