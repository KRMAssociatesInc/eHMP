...
function handle(log, config, environment, job, handlerCallback) {
    ...
    if (!job.patientIdentifier || !job.patientIdentifier.type || job.patientIdentifier.type !== 'pid' || !/^DOD;/.test(job.patientIdentifier.value)) {
        ...
        return setTimeout(handlerCallback, 0, 'jmeadows-sync-domain-request-handler.handle: Expected DOD pid which contains EDIPI as patient id, but it was not found.');
    }
    ...
    if (domainConfig === null) {
        ...
        return setTimeout(handlerCallback, 0, 'jmeadows-sync-domain-request-handler.handle: No configuration for domain');
    }
