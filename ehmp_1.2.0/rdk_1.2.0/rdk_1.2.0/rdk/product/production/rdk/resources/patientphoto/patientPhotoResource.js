...
var searchUtil = require('../patientsearch/searchUtil');
...
function getPatientPhoto(req, res) {
    ...
    if (mviProtocol === 'http' || mviProtocol === 'https') {
        ...
        var xml_1309 = fs.readFileSync('./resources/patientphoto/xml/1309.xml').toString();
        ...
        try {
            ...
            http.post(mviSoapRequest, mviHttpConfig, function(statusCode, data) {
                                ...
                                if (_.isString(vhicId)) {
                                    ...
                                    if (_.isString(vhicIdOnly)) {
                                        ...
                                        if (vhicProtocol === 'http' || vhicProtocol === 'https') {
                                            ...
                                            var xml_vhic = fs.readFileSync('./resources/patientphoto/xml/vhic.xml').toString();
                                            // Replace the variables inside the XML with the corresponding substitution values
...
function sendSoapRequestToVhic(vhicSoapRequest, vhicHttpConfig, req, res) {
    try {
        // Send the SOAP request to VHIC
        http.post(vhicSoapRequest, vhicHttpConfig, function(statusCode, data) {
            if (statusCode === undefined) {
                var results;
                var patientPhoto;
                parseString(data, function(err, result) {
