...
var apiDocs = {
    spec: {
        ...
        summary: 'Converts the vpr \'nonVA\' medication resource into the FHIR \'medicationstatement\' resource.',
        notes: '',
        method: 'GET',
        parameters: [
...
function convertToFhir(inputJSON) {
    ...
    for (var i = 0, l = items.length; i < l; i++) {
        outJSON.push(ra.outpatientmedsFactory('MedicationStatementItem', items[i]));
    }
    return outJSON;
}
...
function buildBundle(results, req) {
    ...
    if (req) {
        ...
        if (req._pid !== undefined) {
            b.title = 'MedicationStatement with subject.identifier \'' + req._pid + '\'';
        }
    }
    ...
    for (var i in results) {
        if (nullchecker.isNotNullish(results[i])) {
            ...
            e.title = 'MedicationStatement for patient [' + req._pid + ']';
            e.id = constants.conditionsFhir.CONDITION_PREFIX + (e.content._id || e.content.id || 'null');
            e.link.push(new fhirResource.Link(req.protocol + '://' + req.headers.host + '/fhir/medicationstatement/' + (e.content._id || e.content.id || '@null'), 'self'));
