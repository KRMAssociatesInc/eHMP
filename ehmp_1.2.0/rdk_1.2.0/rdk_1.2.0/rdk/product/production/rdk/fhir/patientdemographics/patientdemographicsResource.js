...
var apiDocs = {
    spec: {
        ...
        method: 'GET',
        parameters: [
            rdk.docs.swagger.paramTypes.path('id', 'patient id', 'string')
        ],
        responseMessages: []
    }
};
...
function convertToFhir(result) {
    ...
    var genderItem = {};
    genderItem.system = 'http://hl7.org/fhir/v3/AdministrativeGender';
    if (data.genderCode !== undefined) {
        switch (data.genderCode) {
            case 'urn:va:pat-gender:M':
                {
                    genderItem.code = 'M';
                    genderItem.display = 'Male';
                }
                break;
            case 'urn:va:pat-gender:F':
                {
                    genderItem.code = 'F';
                    genderItem.display = 'Female';
                }
                break;
            case 'urn:va:pat-gender:U':
                {
                    genderItem.code = 'UN';
                    genderItem.display = 'Undifferentiated';
                }
                break;
        }
    }
    genderFhir.coding.push(genderItem);
