...
describe 'Story#:1100: Pilot_Patient_Search.rb', acceptance: true do
  ...
  context "AC#1001|TC#1201: Global Search happy path and negative scenarios. \n" do
    ...
    patient_search_scenario.each do |scenario|
      if scenario[:errMsg] == ''
        ...
        meta_data = scenario[:facility].upcase + '|' + scenario[:lName] + '|' + scenario[:fName].split('|')[0] + '|' + scenario[:dob].split('|')[0] + '|' + scenario[:ssn].split('|')[0] + '|' + scenario[:errMsg]
      end
      ...
      it log_text + meta_data do
        ...
        if scenario[:errMsg] == '' || scenario[:errMsg] == 'RESTRICTED RECORD'
          ...
          @over_view.screenNm_element.when_visible(20)
          # Verify that the data displayed in Overview are accurate
          temp_arry = @over_view.oVDoB.split(' (')
          ...
          expect(@over_view.oVSsn.include?(scenario[:ssn].split('|')[1])).to eq(true)
          expect(@over_view.oVGender).to eq(scenario[:gender])
