...
describe('testCipherPad', function() {
    it('tests the CIPHER PAD length', function() {
        expect(VistaJSLibrary.CIPHER_PAD.length).to.equal(20);
    });
...
describe('testRpcParameter', function() {
    ...
    it('tests that Encrypted RpcParameters are built correctly', function() {
        ...
        expect(param).to.equal('0008 CTgF#b!f');

        param = VistaJSLibrary.buildEncryptedParamString('lgs413', 1, 19);
        expect(param).to.equal('0008!S;|\\M&3f');

        param = VistaJSLibrary.buildEncryptedParamString('lgs413', 12, 17);
        expect(param).to.equal('0008,P2GzBT1f');
    });
});

describe('testParamList2String', function() {
    var param_list = [{
        key: '"FILE"',
        value: '200'
    }, {
...
describe('strPack', function() {
    ...
    it('pads string with zeros', function() {
        expect(VistaJSLibrary.strPack('something!5here', 10)).to.equal(expected);
    });
});

describe('testPrependCount', function() {
    var expected = '\nSomeString';
    it('prepends length to string', function() {
        expect(VistaJSLibrary.prependCount('SomeString', 10)).to.equal(expected);
    });
});

describe('testAdjustForNumericSearch', function() {
    var expected = 244;
    it('tests the adjustment for numeric search', function() {
        expect(VistaJSLibrary.adjustForSearch('245')).to.equal(expected);
    });
});

describe('testAdjustForStringSearch', function() {
    var expected = 'Snurc~';
...
describe('testAdjustForSearch', function() {
    var expected = 244;
...
describe('testIsNumeric', function() {
    it('validates various numeric tests', function() {
        expect(VistaJSLibrary.isNumeric('1234')).to.be.true();
        expect(VistaJSLibrary.isNumeric('b1234')).not.to.be.true();
        expect(VistaJSLibrary.isNumeric('1234b')).not.to.be.true();
        expect(VistaJSLibrary.isNumeric('12b34')).not.to.be.true();
        expect(VistaJSLibrary.isNumeric(' 1234')).not.to.be.true();
        expect(VistaJSLibrary.isNumeric('1234 ')).not.to.be.true();
    });
});
