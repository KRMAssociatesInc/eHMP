...
function createOrders(item, req, parentUid) {
    ...
    if (nullchecker.isNotNullish(item.providerName)) {
        var contPractProvider = new fhirResource.Practitioner_DSTU1(helpers.generateUUID());
        ...
        if (nullchecker.isNotNullish(item.providerUid)) {
            contPractProvider.identifier =
                [new fhirResource.Identifier_DSTU1(item.providerUid, undefined, undefined, 'provider-uid')];
        }
    ...
    if (nullchecker.isNotNullish(item.locationUid)) {
        ...
        contLocLocation.identifier =
            new fhirResource.Identifier_DSTU1(item.locationUid, undefined, undefined, 'location-uid');
    ...
    if (nullchecker.isNotNullish(item.facilityCode)) {
        ...
        contOrgFacility.identifier =
            [new fhirResource.Identifier_DSTU1(item.facilityCode, undefined, undefined, 'facility-code')];
    ...
    if (nullchecker.isNotNullish(item.clinicians)) {
        ...
        _.each(item.clinicians, function(clinician) {
            ...
            contPractClinician.identifier = [new fhirResource.Identifier_DSTU1(clinician.uid, undefined, undefined, 'uid')];
...
function createDiagnosticOrder(item, order) {
    ...
    if (nullchecker.isNotNullish(item.uid)) {
        ...
        diagOrder.identifier.push(new fhirResource.Identifier_DSTU1(item.uid, undefined, undefined, 'uid'));
    }
    diagOrder.item = [];
    var itemOI = {
...
function createMedicationPrescription(item, order) {
    ...
    if (nullchecker.isNotNullish(item.uid)) {
        ...
        medPrescription.identifier.push(new fhirResource.Identifier_DSTU1(item.uid, undefined, undefined, 'uid'));
    }
