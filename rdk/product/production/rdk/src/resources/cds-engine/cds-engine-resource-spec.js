...
describe('CDS Engine Resource', function() {
    var resources = cdsengineResource.getResourceConfig();
    var interceptors = {
        audit: true,
        metrics: true,
        authentication: true,
    ...
    it('has correct configuration for CDS Engine get ', function() {
        var r = resources[0];
        expect(r.name).to.equal('CDS-engine-get');
    ...
    it('has correct configuration for CDS Engine post ', function() {
        var r = resources[1];
        expect(r.name).to.equal('CDS-engine-post');
        expect(r.path).to.equal('/registry');
    ...
    it('has correct configuration for CDS Engine put ', function() {
        var r = resources[2];
        expect(r.name).to.equal('CDS-engine-put');
        expect(r.path).to.equal('/registry');
    ...
    it('has correct configuration for CDS Engine delete', function() {
        var r = resources[3];
        expect(r.name).to.equal('CDS-engine-delete');
        expect(r.path).to.equal('/registry');
