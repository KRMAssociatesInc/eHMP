...
function performWriteBack(req, res) {
    ...
    if (eventFilemanYear !== -1) {
        ...
        res.status(rdk.httpstatus.internal_server_error).rdkSend(errorMessage1);
        return;
    }
    ...
    if (inputAllergyParam.historicalOrObserved === OBSERVED &&
        ...
        res.status(rdk.httpstatus.internal_server_error).rdkSend(observedAllergyMissingParamError);
        return;
        ...
        if (typeof inputAllergyParam.observedDate !== 'undefined') {
            ...
            if (observedDate !== -1) {
                ...
                res.status(rdk.httpstatus.internal_server_error).rdkSend(errorMessage2);
                return;
            }
        }
    ...
    VistaJS.callRpc(req.logger, rpcConfig, 'ORWDAL32 SAVE ALLERGY', [allergy.newAllergyIndicator, allergy.dfn, allergy.allergyArray], function(error, result) {
        if (error) {
            ...
            if (returnMessage[0] === '0') {
                ...
                res.status(rdk.httpstatus.internal_server_error).rdkSend((returnMessage && typeof returnMessage[1] !== undefined) ? returnMessage[1] : errorMessage);
            }
        }
    });
}
