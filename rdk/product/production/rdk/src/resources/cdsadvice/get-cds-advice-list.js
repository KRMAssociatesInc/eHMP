...
var errorReadStatusMissingRequired = 'Missing required parameters. The following parameters are required: id, value.';
...
var apiDocs = {
    spec: {
        ...
        parameters: [
            ...
            rdk.docs.swagger.paramTypes.query('readStatus', 'Read status flag', 'string', true)
        ],
        responseMessages: []
    }
};
...
function transformAdviceList(adviceList, common) {
    if (nullchecker.isNotNullish(adviceList)) {
...
function transformWorkProductList(wpList, common) {
    if (nullchecker.isNotNullish(wpList)) {
...
function transformReminders(reminders, common) {
    if (nullchecker.isNotNullish(reminders)) {
...
function getRulesResultsList(req, res, pid, use, callback) {
    ...
    httpUtil.post(content, req.app.config, config, function(err, response) {
        ...
        if (err || !response) {
            ...
            if (response.status && response.status.code !== '0' /* 0 == OK*/ ) {
                ...
                callback(null, response.results || []); // send empty list if results is null
            }
        }
    });
}
 ...
 *
 * @api {get} /resource/cds/advice/list?pid=123VQWE&use=test&readStatus=all Request CDS Advice list
 *
 * @apiName AdviceList
 * @apiGroup CDS Advice
 *
...
function getCDSAdviceList(req, res, next) {
    ...
    if (useCachedValue === 'true' && cachedObj && cachedObj.readStatus === readStatus) {
        ...
        res.status(rdk.httpstatus.ok).rdkSend(cachedObj.data);
    } else {
        async.parallel([
                ...
                function(callback) {
                    ...
                    cdsWorkProduct.retrieveWorkProductsForProvider(provider, pid, readStatus, callback);
                }
            ],
...
/**
 * @api {put} /resource/cds/advice/:id/status/read/:value Set read status
...
function setReadStatus(req, res) {
    ...
    if (nullchecker.isNullish(id) || nullchecker.isNullish(readStatus) || nullchecker.isNullish(provider)) {
        req.logger.error('CDS Read Status - ' + errorReadStatusMissingRequired);
        res.status(rdk.httpstatus.bad_request).rdkSend(errorReadStatusMissingRequired);
    } else {
        cdsWorkProduct.setReadStatus(id, readStatus, provider, function(body, error) {
            if (error) {
                req.logger.error('CDS Read Status - ' + error);
                return res.status(rdk.httpstatus.not_found).rdkSend(errorReadStatusMissingRequired);
            }
