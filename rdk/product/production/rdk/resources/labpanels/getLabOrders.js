    ...
    async.parallel(
        [
            ...
            function(callback) {
                ...
                var jdsFilterQuery = rdk.utils.jdsFilter.build(jdsFilters);
                if (jdsFilterQuery) {
        ...
        function(err, resultArray) {
            if (err) {
                if (err instanceof OrdersError) {
                    ...
                    return res.send(rdk.httpstatus.internal_server_error, err.message);
                }
            }
            ...
            if (!orderDetails) {
                ...
                if (panelsData) {
                    ...
                    return res.send(panelsData);
                } else {
                    //orders errored and labs never finished, return an error
                    return res.send(rdk.httpstatus.internal_server_error, 'Lab panels resource unable to lookup lab orders.');
                }
