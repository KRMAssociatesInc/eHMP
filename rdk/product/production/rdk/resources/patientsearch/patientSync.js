...
var xml_1309 = fs.readFileSync('./resources/patientsearch/xml/1309.xml').toString();
...
function getPatientFromMVI(request, processor, cb) {
    ...
    http.post(soapRequest, httpConfig, function(err, data) {
        if (err) {
...
function getJDSPatientSyncStatus(request, response) {
    ...
    var syncConfig = request.app.config.jdsSync.syncPatientStatus;
    var pid = request.body.pid || ''; //if !pid then error
    syncConfig.options = searchUtil.merge(syncConfig.options, request.app.config.vxSyncServer);
    request.app.subsystems.jdsSync.fetchPatientSyncStatus(syncConfig, pid, request.logger, function(err, result) {
        ...
        if (err) {
            ...
            if (err === 404) {
                ...
                response.send(500, 'There was an error processing your request. The error has been logged.');
            }
        } else {
...
function syncJDSPatient(req, idList, cb) {
    ...
    config.options.path = '/sync/demographicSync';
    /*
    ...
    */
    ...
    http.post(payload, config, callback);
}
