...
function getMatchingSites(req, res, patientData, callback) {
    ...
    if (callback && typeof (callback) === 'function') {
        ...
        res.status(rdk.httpstatus.ok).json(sitesInfo);
    }
}
...
function getSitesInfoFromPatientData(req, res) {
    ...
    httpUtil.fetch(httpConfig, function (err, response) {
        if (err || !response) {
            req.logger.info(err);
            res.status(rdk.httpstatus.internal_server_error).json({
                msg: 'There was an error getting patient data.'
            });
        } else {
            try {
                var patientData = JSON.parse(response);
                if (patientData.error && patientData.error.code === 404) {
                    res.status(rdk.httpstatus.not_found).json({
                        msg: 'Bad Patient ID'
                    });
                    ...
                    getMatchingSites(req, res, patientData, function (sites) {
                        ...
                        getReportListForAllSites(req, res, sites, function (sitesInfo) {
                            if (sitesInfo.error) {
                                res.status(sitesInfo.statuscode).json({
                                    'msg': sitesInfo.error
                                });
                            } else {
                                res.status(rdk.httpstatus.ok).json(sitesInfo);
                ...
                req.logger.error(e);
                res.status(rdk.httpstatus.internal_server_error).json({
                    msg: 'There was an error processing patient data'
                });
            }
        }
    });
...
function getReportContentByReportID(req, res) {
    ...
    if (_.isUndefined(req.param('pid'))) {
        ...
        if (reportId.indexOf(';') === -1) {
            res.status(rdk.httpstatus.internal_server_error).json({
                msg: 'Report Id is not in correct format'
            });
            return;
        }
        ...
        VistaJS.callRpc(req.logger, getVistaRpcConfiguration(req.app.config, site), REPORTCONTENTRPC, params, function (error, result) {
            ...
            if (error) {
                ...
                res.status(rdk.httpstatus.internal_server_error).json(errorObj);
                ...
                if (result.indexOf('Report not Available') > -1) {
                    ...
                    res.status(rdk.httpstatus.not_found).json(errorObj);
