...
class JsonVerifier
  ...
  def build_subarray(fieldpath, fieldvalues, jsonobject)
    ...
    begin
      ...
      @@output.push("exception was thrown and caught: #{e}")
      @@defined_error_msg = e
      returnvalue = false
    end
    return returnvalue
  ...
  def array_is_in_order(fieldpath, fieldvalues, jsonobject)
    returnvalue = true
    begin
      ...
      result_of_compare = fieldvalues <=> allvalues
      p "result was #{result_of_compare}"
      returnvalue = result_of_compare == 0
      unless returnvalue
        @@defined_error_msg = "comparison of 2 arrays failed: #{fieldvalues} to #{allvalues}"
      end
    rescue Exception=>e
      p "#{e}"
      @@defined_error_msg = e
