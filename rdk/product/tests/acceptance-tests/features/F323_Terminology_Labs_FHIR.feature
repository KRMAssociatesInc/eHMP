@terminology @F323 @FHIR @future @DE1010
...
Scenario: An authorized user can VA access Laboratory Chem and see standardized LOINC values when defined through FHIR API
    ...
    And the FHIR results contain "labs terminology from DOD Ncid and LOINC"
      | field                                 | value                                                             |
      #Loinc code
      | content.contained.name.coding.code    | 13955-0                                                           |
...
@terminology_labs_ch @FHIR 
Scenario: An authorized user can DoD access Laboratory Chem and see standardized LOINC values when defined through FHIR API
	Given a patient with "labs" in multiple VistAs
