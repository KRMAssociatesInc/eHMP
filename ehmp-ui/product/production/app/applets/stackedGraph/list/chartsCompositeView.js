    ...
    var ChartsCompositeView = Backbone.Marionette.CompositeView.extend({
        ...
        reorderRows: function(target, reorderObj){
            ...
            if(reorderObj.oldIndex !== reorderObj.newIndex){
                ...
                this.collection.add(temp, {at: reorderObj.newIndex});
        ...
        onDestroy: function(){
            ...
            $('#' + this.options.instanceId + '-scroll-top').remove();
            $('#' + this.options.instanceId + '-scroll-bottom').remove();
        },
        onShow: function() {
            var self = this;
            ...
            var gridAppletPanel = $('#' + this.options.instanceId).find('.grid-applet-panel').first();
            var elementToScroll = $(this.el).find('.collectionContainer').first();
            $('<div id="' + this.options.instanceId + '-scroll-bottom" style="position: relative; top: -50px; height: 10px; background-color: white;"/>').insertAfter($(gridAppletPanel)).on('dragenter', function(e){
                self.bottomInterval = setInterval(function(){
                    var newScrollTop = $(elementToScroll).scrollTop();
                    $(elementToScroll).scrollTop(newScrollTop + 10);
                }, 25);
            }).on('dragleave', function(e){
                self.bottomInterval && clearInterval(self.bottomInterval);
            });

            $('<div id="' + this.options.instanceId + '-scroll-top" style="position: relative; top: 50px; height: 10px; z-index: 22;"/>').insertBefore($(this.el).find('.collectionContainer')).on('dragenter', function(e){
                        ...
                        return function findMatches(q, cb) {
                            ...
                            $.each(strs, function(i, str) {
                                if (substrRegex.test(str)) {
                                    ...
                                    if (_.indexOf(self.allReadyAdded, str.toUpperCase()) === -1) {
                                ...
                                .on('typeahead:selected', function(e, suggestion, dataset) {
                                    ...
                                    if (suggestion.allReadyAdded) {
                                        ...
                                        var model = self.collection.find(function(model) {
                                            return model.get('typeName') === suggestion.value.toUpperCase();
                                        });
