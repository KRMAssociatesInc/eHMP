...
GETOBJ(RESULT,ARGS) ; gets an object given a UID
 ...
 S PID=""
 S ID=""
 S JPID=$$JPID4PID(ARGS("pid"))
 D PID4JPID(.PIDS,JPID)
 ; Loop through patient identifiers and ensure patient identifier is associated with the UID
 ; If patient identifier is not associated quit without returning the object
 F  S ID=$O(PIDS(ID)) Q:ID=""  Q:$L(PID)  D
 . I PIDS(ID)=$O(^VPRPTJ("KEY",ARGS("uid"),"")) S PID=PIDS(ID) Q
 ; If the PID doesn't exist after the loop we know the key won't exist
 ; Return the message clients expect based on prior behavior (Bad Key)
 I '$L(PID) D SETERROR^VPRJRER(104,"Pid:"_ARGS("pid")_" Key:"_ARGS("uid")) Q
...
JPID4PID(PID) ; Return JPID for a PID
 ...
 Q $G(^VPRPTJ("JPID",PID))
 ;
JPIDEXISTS(JPID) ; V4W/DLW - Return true if we have a JPID, false if not
