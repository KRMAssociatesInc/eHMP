...
SETGETNUM ;; @TEST Operational data store/retrieve with fully numeric metastamp
 ...
 S ARG("id")="1234"
 S RETURN=$$SETOD^VPRSTATUS(.ARG,.BODY)
 D ASSERT(11,$D(^VPRSTATUSOD("1234",20150717104657)),"CALL TO SETOD^VPRSTATUS FAILED WITH AN ERROR")
 D ASSERT(1,$D(^VPRSTATUSOD("1234",20150717104657))#10,"Source metastamp doesn't exist")
 D ASSERT(1,$D(^VPRSTATUSOD("1234",20150717104657,"asu-class",20150717104657)),"Domain: asu-class metastamp doesn't exist")
 D ASSERT(1,$D(^VPRSTATUSOD("1234",20150717104657,"asu-class","urn:va:asu-class:1234:100",20150717102913)),"Item metastamp 'urn:va:asu-class:DCBA:100' doesn't exist")
 ; Retrieve the data
 S ARG("id")="1234"
 S ARG("detailed")="true"
 D GETOD^VPRSTATUS(.DATA,.ARG)
 ...
 D ASSERT(10,$D(OBJECT("inProgress")),"Sync status is not completed")
 D ASSERT(0,$D(OBJECT("completedStamp")),"Sync status is not completed")
 D ASSERT(0,$D(OBJECT("inProgress","sourceMetaStamp","""1234","domainMetaStamp","asu-class","syncCompleted")),"asu-class domain should not be complete")
 ...
 D ASSERT(1,$G(^VPRSTATUSOD(1234,20150717104657,"asu-class","urn:va:asu-class:1234:100",20150717102913,"stored")))
 ; Delete all of the data
 K ARG,BODY,RETURN
 D DELOD^VPRSTATUS(.BODY,.ARG)
 D ASSERT(0,$D(^VPRSTATUSOD(1234,20150717104657)),"CALL TO DELOD^VPRSTATUS FAILED WITH AN ERROR")
 Q
