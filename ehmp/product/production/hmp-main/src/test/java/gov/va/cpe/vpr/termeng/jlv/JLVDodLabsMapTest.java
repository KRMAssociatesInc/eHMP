...
public class JLVDodLabsMapTest {
	...
	JLVDodLabsMap testSubject = null;
	...
	public void setUp() throws Exception {
		...
		testSubject = new JLVDodLabsMap(mockConnection);
	...
	private void verifyGeneratedContent(JLVMappedCode oMappedCode) {
		...
		assertEquals("The code system was incorrect.", JLVDodLabsMap.CODE_SYSTEM_LOINC, oMappedCode.getCodeSystem());
	...
	public void testGetLoincInfoFromLoincCodeHappyPath() {
		try {
			...
			when(mockResultSet.getString(JLVDodLabsMap.SQL_FIELD_LOINC_LOINC_NUM)).thenReturn(GENERATED_LOINC_CODE);
			when(mockResultSet.getString(JLVDodLabsMap.SQL_FIELD_LOINC_LONG_COMMON_NAME)).thenReturn(GENERATED_DISPLAY_TEXT);

			JLVMappedCode oMappedCode = testSubject.getLabLoincFromDodNcid(GENERATED_LOINC_CODE);
	...
	public void testLoincInfoFromLoincCodeNoMappingBecauseFieldsEmpty() {
		try {
			when(mockResultSet.next()).thenReturn(true);
			when(mockResultSet.getString(JLVDodLabsMap.SQL_FIELD_LOINC_LOINC_NUM)).thenReturn("");
			when(mockResultSet.getString(JLVDodLabsMap.SQL_FIELD_LOINC_LONG_COMMON_NAME)).thenReturn("");
	...
	public void testLoincInfoFromLoincCodeNull() {
		try {
			...
			JLVMappedCode oMappedCode = testSubject.getLabLoincFromDodNcid(null);
			assertNull("The result should have been null", oMappedCode);
		} 
		catch (Exception e) {
			System.out.println("An unexpected exception occurred.  Error: " + e.getMessage());
	...
	public void testLoincInfoFromLoincCodeCreatingWithNullParameter() {
		try {
			@SuppressWarnings("unused")
			JLVDodLabsMap oDodLabsMap = new JLVDodLabsMap(null);
			fail("There should have been a TermLoadException thrown.");
		}
		catch (TermLoadException e) {
			// We expected this exception.
