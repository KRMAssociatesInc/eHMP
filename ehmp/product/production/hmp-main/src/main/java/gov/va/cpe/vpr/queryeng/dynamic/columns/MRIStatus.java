 ...
 *
 */
@Component(value = "gov.va.cpe.vpr.queryeng.dynamic.columns.MRIStatus")
@Scope("prototype")
public class MRIStatus extends ViewDefDefColDef {
    ...
    public MRIStatus(Map<String, Object> vals) {
		super(vals);
	}
	...
	public String getDescription() {
		return "MRI scan results (based on Imaging Reports) within the specified time period. If no results found, " +
				"any orders (again within the specified time period) of type 'MRI'. If no orders found, provides an " +
    ...
    public Map<String, Object> runDeferred(DeferredBoardColumnTask dtask) {
        ...
        try {
            ...
            RenderTask task = job.getAction(ViewRenderAction.class).getResults();

            Iterator<Map<String, Object>> iter = task.iterator();
            boolean found = false;

            String site = null;
            if(configProperties.get("site")!=null && !configProperties.get("site").equals("")) {site = configProperties.get("site").toString();}

            while(iter.hasNext()) {
                Map<String, Object> itm = iter.next();
                if(itm.get("imagingTypeUid")!=null && itm.get("imagingTypeUid").equals("urn:va:imaging-Type:MAGNETIC RESONANCE IMAGING")) {
            ...
            if(!found) {
                ...
                configProperties.put("filter_group", "MRI");
                configProperties.put("pid", params.get("pid"));

                task = dtask.runner.exec("gov.va.cpe.vpr.queryeng.OrdersViewDef", configProperties).getAction(ViewRenderAction.class).getResults();
                iter = task.iterator();
                while(iter.hasNext()) {
                    Map<String, Object> itm = iter.next();
            ...
            log.error("Cannot render MRIStatus: "+e.getMessage(), e);
