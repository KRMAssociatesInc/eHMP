...
/**
 * This class is used to load the reactant information from the JLV REACTANTS spreadsheet page into
 * the reactants table in the h2 database.
 * 
 * @author Les.Westberg
 */
public class JLVH2ReactantsLoadUtil implements IJLVH2PageLoadUtil {
	private static final String PAGE_NAME = "REACTANTS";
	private static final String CRLF = System.getProperty("line.separator");
	private static final Object TITLE_ROW_CELL_1_TEXT = "UMLSCode";
	private static final String SQL_CREATE_TABLE = "CREATE TABLE IF NOT EXISTS reactants (id INT PRIMARY KEY, " + 
	                                                                                   "umlsCode VARCHAR(20), " + 
	                                                                                   "vuid VARCHAR(20)," +
	                                                                                   "vistaText VARCHAR(500)," +
	                                                                                   "umlsText VARCHAR(500))";
	private static final String SQL_CLEAR_ALL = "DELETE FROM reactants";
	private static final String SQL_INSERT = "INSERT INTO reactants VALUES (?, ?, ?, ?, ?)";
	private static final int SQL_FIELD_ID = 1;
	private static final int SQL_FIELD_UMLS_CODE = 2;
	private static final int SQL_FIELD_VUID = 3;
	private static final int SQL_FIELD_VISTA_TEXT = 4;
	private static final int SQL_FIELD_UMLS_TEXT = 5;
	...
	private JLVH2ReactantsLoadUtil () {
	}
	 ...
	 */
	public JLVH2ReactantsLoadUtil(String sMapFileName, FileInputStream fisMapFile, Connection oDBConnection, PrintWriter pwStatusFile) throws TermLoadException {
	...
	private void addRowToTermDatabase(RowMapping oRowMapping) throws SQLException {
		...
		psInsertStatment.setString(SQL_FIELD_UMLS_CODE, oRowMapping.getUmlsCode());
		...
		psInsertStatment.setString(SQL_FIELD_UMLS_TEXT, oRowMapping.getUmlsText());
	...
	private void outputRowMessage(String sTitle, String sMessage, RowMapping oRowMapping) {
		...
		this.pwStatusFile.println("     umlsCode: " + oRowMapping.getUmlsCode());
		this.pwStatusFile.println("     vuid: " + oRowMapping.getVuid());
		this.pwStatusFile.println("     vistaText: " + oRowMapping.getVistaText());
		this.pwStatusFile.println("     umlsText: " + oRowMapping.getUmlsText());
	}
