...
DPT1OD(PAT) ; -- Demographics (data array only)
 ...
 I $G(HMPMETA)=1 D ADD^HMPMETA("patient",PAT("uid"),PAT("stampTime")) Q  ;US6734
 Q
 ;
LKUP ; patient lookup data
 ...
 I $G(HMPMETA)=1,$P($G(HMPFADOM),"#")="pt-select" D ADD^HMPMETA("pt-select",PAT("uid"),$G(HMPSTMP)) Q  ;US6734
