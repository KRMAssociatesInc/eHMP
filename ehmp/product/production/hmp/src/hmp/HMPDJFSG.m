...
GETSUB(HMPFRSP,ARGS) ; retrieve items from stream
 ...
 ;
 K ^TMP("HMPF",$J)
 N HMPFSYS,HMPFSTRM,HMPFLAST,HMPFDT,HMPFLIM,HMPFMAX,HMPFSIZE
 N HMPFSEQ,HMPFIDX,HMPFCNT,SNODE,STYPE,HMPFERR,HMPDEL,HMPERR,HMPSTGET  ;*S68-JCH*
...
FINISH(HMPDEL,HMPERR) ;reset the FIRST object delimiter, add header and tail
 ...
 ;delete node
 ;I $D(HMPDEL) D  ;                                 *BEGIN*S68-JCH*
 ;.S CLOSE=$S(HMPFCNT:"},",1:""),START=1
 ;.;S HMPFCNT=HMPFCNT+1,^TMP("HMPF",$J,HMPFCNT)=CLOSE_"]}"
 ...
 ;.S HMPFCNT=HMPFCNT+1,^TMP("HMPF",$J,HMPFCNT)="]"  *END*S68-JCH* ;error node
 I $D(HMPERR) D
 .S CLOSE=$S(HMPFCNT:"},",1:""),START=1
...
WRAPPER(DOMAIN,PIDS,OFFSET,DOMSIZE,FROMXTR) ; return JSON wrapper for each item *S68-JCH*
 ...
 .; --- End US3907 ---
 .I DFN'="OPD" D METAPT^HMPMETA(SNODE) Q  ; Collect Patient metastamp data from XTMP - US6734
 .D METAOP^HMPMETA(SNODE) ; Collect OPD metastamp data from XTMP - US6734
 S X=X_","
 ;;PJH;;S X=X_",""batch"":"_$S(+$G(FROMXTR):"true",1:"false") ; if batched by extract  *S68-JCH*
