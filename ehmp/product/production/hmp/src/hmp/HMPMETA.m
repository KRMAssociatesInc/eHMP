HMPMETA ;SLC/PJH -- Utility to collect Domains, UID's and stamptimes for a patient   ;8/14/13  11:22
...
MERGE(HMPBATCH) ;Merge metastamp data into XTMP and mark domain complete in metastamp
 ...
 Q
 ;
METAPT(A) ; MetaStamp for patient data (within its own syncStart chunk).  
 ; --Input parameter
...
UNSUB(DFN,SRV) ;Unsubscribe
 ...
 Q
 ;
DEBUG ; Hang subscription process
 N I F I=1:1 Q:'$D(^ZZPJH("HANG"))  H 60
 Q
