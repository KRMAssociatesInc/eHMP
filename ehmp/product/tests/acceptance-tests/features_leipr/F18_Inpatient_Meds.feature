@Inpatient_Meds
Feature: F18 Inpatient Medications
    If Inpatient Medication data is requested for a patient, or a change to Inpatient Medication data is detected for a patient, that data will be returned for all VistA instances in which Inpatient Medication data exists for that patient, stored in the LEIPR, and displayed in the JLV.

@future
Scenario: VistA Exchange supports request, retrieval, storage, and display of Inpatient Medication data from two VistA instances for a patient
  Given a patient with id "" has not been synced
  When a client requests "" for patient with id ""
  Then a successful response is returned within "60" seconds
  And that patient has Inpatient Medication data at multiple VistA instances
    | field                               | panorama_value              | kodak_value                 |
  
  Then a response is returned with Inpatient Medication data from multiple VistA instances for that patient from LEIPR (Data come from one record)
    ...
    | contained[MedicationPrescription].status                        | 12008                                                         | No  |
    ...
    | contained[MedicationPrescription].dispense.quantity.extension[extendedQuantityUnits].url    | TAB                             | No  |
    | contained[MedicationPrescription].dispense.quantity.extension[extendedQuantityUnits].value     | TAB                             | No  |
