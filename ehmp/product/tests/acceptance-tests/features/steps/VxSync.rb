...
class VxSync
  ...
  def check_pid_if_found_list_not_sync(pid, site_name_list)
    ...
    unless found_pid == nil
      ...
      site_names.each do |site_name|
        fail "The sync for this patient #{pid} has been failed before!" if found_pid.include? site_name
      end
  ...
  def check_status_until_sync_process_completed(path, site_name_list, check_opd_pendeing, max_wait_time = 90)
    ...
    while sync_completed == false && @index < max_index_run && @index_job_status < max_index_run
      ...
      # p "The sync completed for site name: #{site_names} is #{sync_site}"
      # p "The job array is empty: #{sync_jobs}"
      # p "The sync completed and The job array is empty: #{sync_completed}"
  ...
  def check_all_sites_sync_completed(json, site_names)
    ...
    site_names.each do |site_name|
      ...
      if single_site_sync_completed
        ...
        @site_is_not_sync << site_name unless @site_is_not_sync.include? site_name
      end
    end
    # p "the site that sync is not completed yet #{@site_is_not_sync}"
  ...
  def check_single_site_sync_completed(json, site_name)
    ...
    begin
      single_site_sync_completed = json["completedStamp"]["sourceMetaStamp"][site_name]["syncCompleted"]
    rescue
      single_site_sync_completed = false
    end 
    # p "Single site sync completed for #{site_name} is #{single_site_sync_completed}"
    # p json 
    return single_site_sync_completed
  end
  ...
  def comper_old_with_runtime(inprogress_result)
    ...
    site_names.each do |site_name|
      ...
      if inprogress_result[site_name] == @old_inprogress_result[site_name]
        ...
        p "New result is coming for site #{site_name}"
        @index = 0
        @index_job_status = 0
  ...
  def check_pending_sync_jobs(json, site_names)
    json = json["jobStatus"]
    if json == nil
      p "Job Status is NIL"
      return false
    ...
    site_jobs_pending = check_site_jobs_pending(json, site_names)
    if site_jobs_pending
      return true
    end
    return false
  end
  ...
  def comper_old_with_runtime_job_status(job_status_result)
    if job_status_result == @old_job_status_result
      ...
      p "New result is coming for Job status"
      @index = 0
      @index_job_status = 0
  ...
  def clear_request(path)
    ...
    begin
      response = HTTPartyWithBasicAuth.post_with_authorization(path, default_timeout)
      success_result = true
    rescue Exception => e
      puts "An error has occured at patient data clear request."
      puts e
      response = e
      success_result = false
    end
